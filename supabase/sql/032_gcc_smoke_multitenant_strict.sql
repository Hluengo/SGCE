-- =============================================================================
-- 032_gcc_smoke_multitenant_strict.sql
-- Version STRICT del smoke GCC multi-tenant.
--
-- Comportamiento:
-- - Ejecuta inserts de prueba en tablas GCC v2.
-- - Registra checks en tabla temporal.
-- - Si hay FAIL, lanza EXCEPTION.
-- - Todo termina en ROLLBACK (no deja datos).
-- =============================================================================

begin;

-- Config: poner true si quieres exigir contexto completo en tenant B.
do $$
begin
...
insert into _gcc_checks(check_name, status, details)
select
  'enum_estado_legal_has_pausa_legal',
  case
    when exists (
      select 1
      from pg_enum e
...
with rows_src as (
  ...
  select
    ...
    jsonb_build_object('source', 'smoke_032_strict')
  from rows_src r
  returning id, establecimiento_id
)
insert into _gcc_ids(table_name, tenant_id, id)
...
with rows_src as (
  ...
  select
    ...
    r.mediacion_id,
    'CONSTANCIA',
    jsonb_build_object('nota', '[STRICT] Acta GCC'),
    false,
    current_date,
    '[STRICT] Observaciones acta'
  from rows_src r
  returning id, establecimiento_id
)
insert into _gcc_ids(table_name, tenant_id, id)
...
begin
  ...
  insert into _gcc_checks(check_name, status, details)
  values (
    'update_transition:mediaciones_gcc_v2',
...
begin
  ...
  if coalesce(v_fail_count, 0) > 0 then
    raise exception using
      errcode = 'P0001',
      message = format('STRICT GCC SMOKE TEST FAILED (%s checks)', v_fail_count),
