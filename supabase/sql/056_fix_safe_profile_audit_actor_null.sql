...
-- Corrige error NOT NULL en superadmin_audit_logs.actor_user_id cuando
-- safe_* se ejecuta desde SQL Editor (auth.uid() = NULL).
-- ============================================================================

BEGIN;

CREATE OR REPLACE FUNCTION public.resolve_admin_actor_user_id()
RETURNS UUID
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  -- Fallback para SQL Editor/servicio: tomar un perfil superadmin activo.
  SELECT p.id
    INTO v_actor
  FROM public.perfiles p
  WHERE p.activo = true
    AND p.rol::text IN ('superadmin', 'admin')
  ORDER BY CASE WHEN p.rol::text = 'superadmin' THEN 0 ELSE 1 END, p.updated_at DESC
  LIMIT 1;
...
GRANT EXECUTE ON FUNCTION public.resolve_admin_actor_user_id() TO authenticated;
...
BEGIN
  IF NOT public.is_superadmin_or_admin_context() THEN
  ...
  v_actor_user_id := public.resolve_admin_actor_user_id();
  IF v_actor_user_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'AUDIT_ACTOR_NOT_RESOLVED');
  END IF;
