...
export const GccDashboard: React.FC<GccDashboardProps> = ({ baseMetrics }) => {
  ...
  const mecanismoFilterId = 'gcc-filter-mecanismo';
  const estadoFilterId = 'gcc-filter-estado';
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="bg-white border border-blue-100 rounded-2xl shadow-sm p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 lg:gap-4">
          <div className="rounded-lg md:rounded-xl border border-blue-200 bg-blue-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-blue-700 uppercase tracking-tight">Casos Activos</p>
          ...
          <div className="rounded-lg md:rounded-xl border border-red-200 bg-red-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-red-700 uppercase tracking-tight">Casos Vencidos</p>
            <div className="flex items-baseline gap-2">
          ...
          <div className="rounded-lg md:rounded-xl border border-rose-200 bg-rose-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-rose-700 uppercase tracking-tight">Alerta T1</p>
            <p className="text-lg md:text-xl font-black text-slate-900">{metrics.t1}</p>
            <p className="text-[11px] md:text-xs text-rose-600">Vence mañana</p>
          </div>
          
          <div className="rounded-lg md:rounded-xl border border-amber-200 bg-amber-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-amber-700 uppercase tracking-tight">Alerta T2</p>
            <p className="text-lg md:text-xl font-black text-slate-900">{metrics.t2}</p>
            <p className="text-[11px] md:text-xs text-amber-600">Vence en 2 días</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white border border-indigo-100 rounded-2xl shadow-sm p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 lg:gap-4 mb-4 md:mb-6">
          <div className="col-span-1 rounded-lg md:rounded-xl border-2 border-indigo-200 bg-indigo-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-indigo-700 uppercase tracking-tight">Tasa de Adopción</p>
          ...
          <div className="col-span-1 rounded-lg md:rounded-xl border-2 border-emerald-200 bg-emerald-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-emerald-700 uppercase tracking-tight">Más Utilizado</p>
          ...
          <div className="col-span-1 sm:col-span-2 lg:col-span-1 rounded-lg md:rounded-xl border-2 border-blue-200 bg-blue-50 p-3 md:p-4">
            <p className="text-[11px] md:text-xs font-black text-blue-700 uppercase tracking-tight">Tendencia vs. Mes Anterior</p>
          ...
          {metrics.mecanismos.map((mecanismo) => {
            ...
            return (
              <div key={mecanismo.mecanismo} className={`rounded-lg md:rounded-xl border ${config.bgClass} p-3 md:p-4`}>
                ...
                <p className="text-[11px] md:text-xs font-black uppercase tracking-tight">{config.label}</p>
                <div className="flex items-baseline gap-1 mt-1.5">
                  <p className="text-xl md:text-2xl font-black text-slate-900">{mecanismo.count}</p>
                  <span className="text-[11px] md:text-xs font-bold text-slate-600">({mecanismo.percentage}%)</span>
                </div>
                {/* Barra de progreso */}
        ...
        <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50/70 p-3 md:p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-black uppercase tracking-wide text-slate-700 hover:bg-slate-100"
              >
              ...
              <button
                ...
                className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-blue-300 bg-blue-50 px-3 py-2 text-xs font-black uppercase tracking-wide text-blue-700 hover:bg-blue-100"
              >
                <Download className="w-4 h-4" />
                Exportar XLSX
              </button>
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-3">
            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
              <label htmlFor={mecanismoFilterId} className="mb-1 inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider text-slate-500">
                <Filter className="w-3 h-3" />
                Mecanismo
              </label>
              <select
                id={mecanismoFilterId}
                value={mecanismoFilter}
                onChange={(event) => setMecanismoFilter(event.target.value as MecanismoGCC | 'ALL')}
                className="w-full min-h-11 rounded-md border border-slate-200 bg-white px-2 py-2 text-xs font-semibold text-slate-700"
              >
            ...
            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
              <label htmlFor={estadoFilterId} className="mb-1 inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider text-slate-500">
                <Filter className="w-3 h-3" />
                Estado
              </label>
              <select
            ...
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2">
              <p className="text-[11px] font-black uppercase tracking-widest text-emerald-700">Registros filtrados</p>
          ...
          <div className="mt-3 space-y-2 md:hidden">
            {filteredRecords.length > 0 ? (
              filteredRecords.slice(0, 20).map((item) => (
                <article key={`${item.casoId}-${item.createdAt}`} className="rounded-lg border border-slate-200 bg-white p-3">
                  <p className="text-xs font-black text-slate-800">{MECANISMO_CONFIG[item.mecanismo].label}</p>
                  <p className="text-xs text-slate-600 mt-1">{item.descripcion}</p>
                  <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-slate-600">
                    <p><span className="font-semibold">Caso:</span> {item.casoId.slice(0, 8)}</p>
                    <p><span className="font-semibold">Clasif:</span> {item.clasificacion}</p>
                    <p><span className="font-semibold">Estado:</span> {item.estadoImplementacion}</p>
                    <p><span className="font-semibold">Fecha:</span> {new Date(item.createdAt).toLocaleDateString('es-CL')}</p>
                  </div>
                </article>
              ))
            ) : (
              <div className="rounded-lg border border-slate-200 bg-white px-3 py-6 text-center text-slate-500 text-sm">
                Sin registros para el filtro aplicado.
              </div>
            )}
          </div>

          <div className="mt-3 hidden md:block overflow-x-auto rounded-lg border border-slate-200 bg-white">
