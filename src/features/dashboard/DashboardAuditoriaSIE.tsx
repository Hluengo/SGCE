...
const DashboardAuditoriaSIE: React.FC = () => {
  ...
  return (
    <main className="flex-1 p-4 md:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-slate-50 overflow-y-auto">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className={`px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest ${kpis.saludGlobal > 85 ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
              Nivel de Cumplimiento
            </div>
          </div>
          <h3 className="text-4xl font-black text-slate-800 tracking-tighter">{kpis.saludGlobal}%</h3>
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Índice de Salud Legal Global</p>
        </div>

        <div className={`bg-white p-6 rounded-3xl border shadow-sm relative overflow-hidden group ${kpis.alertasNulidad > 0 ? 'border-red-200 bg-red-50/20' : 'border-slate-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <div className={`p-4 rounded-2xl ${kpis.alertasNulidad > 0 ? 'bg-red-500 text-white animate-pulse' : 'bg-slate-100 text-slate-400'}`}>
              <AlertOctagon className="w-6 h-6" />
            </div>
            {kpis.alertasNulidad > 0 && (
              <div className="px-3 py-1 bg-red-600 text-white rounded-full text-xs font-black uppercase tracking-widest shadow-lg shadow-red-200">
                Acción Inmediata
              </div>
            )}
          </div>
          ...
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Alertas de Nulidad (Gradualidad)</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Vencimientos &lt; 48 Horas</p>
        </div>
      </section>
      ...
      <section className="bg-white border border-slate-200 rounded-3xl shadow-xl shadow-slate-200/40 overflow-hidden flex flex-col">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex bg-white p-1 rounded-2xl border border-slate-200 shadow-inner">
              <button 
                ...
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${filterRiesgo === 'ALL' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
              >
                Todos
              </button>
              <button 
                onClick={() => setFilterRiesgo('NULIDAD')}
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${filterRiesgo === 'NULIDAD' ? 'bg-red-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
              >
                Riesgo Nulidad
              </button>
              <button 
                onClick={() => setFilterRiesgo('PROPORCIONALIDAD')}
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${filterRiesgo === 'PROPORCIONALIDAD' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
              >
                Baja Salud Legal
              </button>
            </div>
        ...
        <div className="md:hidden p-4 space-y-4">
          {filteredAudit.map((item) => (
            <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-black text-slate-400 uppercase">Expediente</p>
                  <p className="text-xs font-black text-slate-800 uppercase">{item.nnaNombre}</p>
                  <p className="font-mono text-xs text-blue-500 font-bold">{item.id}</p>
                </div>
                <div className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-tighter ${
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-black uppercase">
              ...
              <button
                ...
                className="w-full mt-2 py-2 bg-white border border-slate-200 rounded-xl text-xs font-black uppercase text-slate-600"
              >
                Ver detalle
              </button>
            </div>
          ...
          {filteredAudit.length === 0 && (
            <div className="py-4">
              <AsyncState
                ...
                message="No hay expedientes para el filtro de riesgo seleccionado."
                compact
              />
            </div>
          )}
                ...
                <tr key={item.id} className="hover:bg-slate-50/80 transition-all group">
                  <td className="px-4 md:px-8 py-6">
                    <div className="flex flex-col">
                      ...
                      <span className="font-mono text-xs text-blue-500 font-bold">{item.id}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 md:px-8 py-6">
                    <div className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-black uppercase tracking-tighter ${
                      item.status === 'READY' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
          ...
          {filteredAudit.length === 0 && (
            <div className="py-8 px-4">
              <AsyncState
                state="empty"
                title="Sin resultados"
                message="No se encontraron expedientes con este nivel de riesgo."
                compact
              />
            </div>
          )}
      ...
      <section className="bg-slate-900 rounded-3xl p-4 md:p-10 text-white relative overflow-hidden group">
        <div className="absolute right-0 top-0 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl -mr-48 -mt-48 group-hover:bg-blue-600/20 transition-all"></div>
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10 relative z-10">
          <div className="space-y-4 max-w-2xl">
            ...
            <div className="flex space-x-4 pt-4">
              <button className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all">
                Revisar Alertas Críticas
              </button>
              <button className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all">
                Ver Normativa 782
              </button>
            </div>
          </div>
          <div className="shrink-0 p-8 border border-white/10 rounded-3xl bg-white/5 backdrop-blur-sm">
             <Gavel className="w-16 h-16 text-blue-400 mb-4 mx-auto" />
             <p className="text-xs font-black uppercase tracking-widest text-center">Compliance Escolar v2.5</p>
          </div>
        </div>
      </section>
    </main>
