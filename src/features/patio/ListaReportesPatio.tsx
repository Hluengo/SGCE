...
const ListaReportesPatio: React.FC = () => {
  ...
  const handleDerivar = async (reporteId: string) => {
    ...
    if (!reporte.estudiante_id) {
      setActionStatus({ type: 'error', message: 'No se puede derivar: el reporte no tiene estudiante asociado.' });
      return;
    }
    ...
    if (!userId) {
      setActionStatus({ type: 'error', message: 'Debes iniciar sesión para derivar a expediente.' });
      return;
    }

    setActionStatus({ type: 'info', message: 'Derivando reporte a expediente...' });
    ...
    try {
      ...
      if (searchError) {
        ...
        setActionStatus({ type: 'error', message: `Error al buscar expedientes: ${searchError.message}` });
        return;
      }
      ...
      if (insertError || !inserted?.id) {
        ...
        setActionStatus({ type: 'error', message: `No se pudo crear expediente: ${insertError?.message ?? 'sin detalle'}` });
        return;
      }
    ...
    if (reporteError) {
      setActionStatus({ type: 'error', message: `Se creó/vinculó expediente, pero no se pudo actualizar reporte: ${reporteError.message}` });
      return;
    }
    ...
    setReportes((prev) =>
      prev.map((r) =>
        r.id === reporteId
          ? { ...r, estado: 'DERIVADO', expediente_id: expedienteDbId, accion_tomada: `Derivado a expediente ${expedienteFolio}` }
          : r
      )
    );
    setActionStatus({ type: 'success', message: `Reporte derivado correctamente al expediente ${expedienteFolio}.` });
    navigate(`/expedientes/${expedienteFolio}`);
    } catch (error) {
      console.error('Error inesperado en derivación:', error);
      setActionStatus({ type: 'error', message: `Error inesperado: ${error instanceof Error ? error.message : 'sin detalle'}` });
    }
  };
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              {reporte.estado === 'DERIVADO' && reporte.expediente_id && (
                <button
                  onClick={() => void handleIrAExpediente(reporte.expediente_id as string)}
